...
cmd({
    pattern: "codex",
   desc: "AI Natural Language Group Control",
   category: "group",
    filename: __filename
},
async (conn, m, { text, isCreator, isAdmins, isBotAdmins }) => {
   if (!text) return m.reply("⟮ CODEX ⟯: System online. Awaiting natural language input.");
  if (!m.isGroup) return m.reply("⟮ CODEX ⟯: Error: Group Environment Required.");
   if (!isAdmins && !isCreator) return m.reply("⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Access Denied.");

   const aiResponse = await getCodexResponse(text); 

  if (aiResponse.includes('[CMD:')) {
       const cmdTag = aiResponse.match(/\[CMD:(.*?)\]/)[1];
       const [action, value] = cmdTag.split(' ');

       try {
           // --- GROUP MUTE/UNMUTE ---
          if (action === 'mute') {
              if (!isBotAdmins) return m.reply("⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Admin privileges required.");
               await conn.groupSettingUpdate(m.chat, 'announcement');
               return m.reply("⟮ CODEX ⟯: Group protocol set to: CLOSED.");
          }
          if (action === 'unmute') {
              await conn.groupSettingUpdate(m.chat, 'not_announcement');
               return m.reply("⟮ CODEX ⟯: Group protocol set to: OPEN.");
           }

          // --- ANTILINK PROTOCOL ---
           if (action === 'antilink_on') {
              if (!isBotAdmins) return m.reply("⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Admin privileges required.");
                await conn.sendMessage(m.chat, { text: "⟮ CODEX ⟯: Antilink protocol: ENABLED." });
               return await conn.sendMessage(m.chat, { text: `.antilink on` });
            }

           // --- ANTIGROUPMENTION PROTOCOL ---
           if (action === 'antimention_on') {
               if (!isBotAdmins) return m.reply("⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Admin privileges required.");
               await conn.sendMessage(m.chat, { text: "⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Antigroupmention protocol: ENABLED." });
              // Triggers Anita's native antigroupmention command
               return await conn.sendMessage(m.chat, { text: `.antigroupmention on` });
          }
          if (action === 'antimention_off') {
               await conn.sendMessage(m.chat, { text: "⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Antigroupmention protocol: DISABLED." });
              return await conn.sendMessage(m.chat, { text: `.antigroupmention off` });
           }

      } catch (err) {
           return m.reply("⟮ 𝙲𝙾𝙳𝙴𝚇 ⟯: Execution Error.");
      }
  }

   const cleanMsg = aiResponse.replace(/\[CMD:.*?\]/g, '').trim();
   return m.reply(`*───「 CODEX 」───*\n\n${cleanMsg}`);
});
